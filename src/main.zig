...
pub fn main(init: std.process.Init) !void {
    ...
    // gemma4-layer0 <model.litertlm> <token_id>: minimum-viable end-to-
    // end single-token forward through layer 0 (no KV history,
    // no per-layer norms, no Q/K-norm, no PLE residual yet).
    // Smoke-tests every CPU kernel chain on real weights.
    if (std.mem.eql(u8, args[1], "gemma4-layer0")) {
        if (args.len < 4) {
            try err.writeAll("usage: vantablack gemma4-layer0 <model.litertlm> <token_id>\n");
            try err.flush();
            return error.MissingArgs;
        }
        const tid = try std.fmt.parseInt(u32, args[3], 10);
        try runGemma4Layer0(gpa, arena, io, args[2], tid, out, err);
        return;
    }
...
fn runGemma4Layer0(
    ...
    try out.print("forwardLayer0SingleToken (token {d}, pos 0)\n", .{token_id});
    try model.forwardLayer0SingleToken(gpa, token_id, h);
    try summarize(out, "  hidden_after_L0", h);
    try out.flush();
}
